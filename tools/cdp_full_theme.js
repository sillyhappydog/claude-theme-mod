...
function buildContentScript() {
  ...
  // Override --bg-100 on .dframe-content-inner — this selector has higher specificity than :root
  // and sets --bg-100: var(--df-bg-page-hsl) which breaks gradient colors
  if (bgMainHSL) {
    css += `.dframe-content-inner{--bg-100:${bgMainHSL.h} ${bgMainHSL.s}% ${bgMainHSL.l}%!important}`;
  }
  ...
  // Gradient fade overrides — no longer needed, .dframe-content-inner --bg-100 override handles this
...
function persistScript(targetId, script, label) {
  return new Promise((resolve, reject) => {
    const ws = new WebSocket(`ws://localhost:${CDP_PORT}/devtools/page/${targetId}`);
    const timer = setTimeout(() => { ws.close(); reject(new Error(`${label}: timeout`)); }, 10000);
    let msgId = 0;
    ws.on('open', () => {
      ...
      // Then register script to run on every new document
      ...
      ws.send(JSON.stringify({ id: ++msgId, method: 'Page.addScriptToEvaluateOnNewDocument', params: { source: wrapped } }));
    });
    ws.on('message', (data) => {
      const r = JSON.parse(data.toString());
      if (r.id === msgId) {
        clearTimeout(timer);
        console.log(`${label}: persistent script registered (id: ${r.result?.identifier})`);
        ws.close();
        resolve(r.result?.identifier);
      }
    });
    ws.on('error', (e) => { clearTimeout(timer); reject(new Error(`${label}: ${e.message}`)); });
  });
...
(async () => {
  try {
    ...
    if (content) {
      await injectViaWS(content.id, buildContentScript(), 'Content');
      await persistScript(content.id, buildContentScript(), 'Content persist');
    }
