        ...
        <div style={{ width: 180, background: t.bgSidebar, borderRight: `1px solid ${t.borderColor}25`, padding: "10px 0", display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden" }}>
          <div style={{ padding: "0 12px", marginBottom: 10 }}>
            {[["Chat", true], ["Code", false], ["Cowork", false]].map(([label, active]) => (
              <span key={label} style={{ fontSize: 11, fontWeight: active ? 700 : 400, color: active ? t.accentPrimary : t.textMuted, marginRight: 12, cursor: "pointer" }}>{label}</span>
            ))}
          </div>
          ...
          {["Let's Talk!", "Honesty is the key to...", "Kaggle glove.6B.50d", "grep ^honesty dim[6]", "have you talked to her?"].map((title, i) => (
            <div key={i} style={{
              ...
              background: i === 0 ? `${t.accentPrimary}15` : "transparent", borderLeft: i === 0 ? `2px solid ${t.accentPrimary}` : "2px solid transparent",
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ maxWidth: "85%" }}>
              ...
              <div style={{ color: t.textPrimary, fontSize: 13, lineHeight: 1.65, marginTop: 8 }}>
                <code style={{ background: `${t.codeBg}cc`, color: t.inlineCodeText, padding: "2px 7px", borderRadius: 4, fontSize: 12, border: `1px solid ${t.accentPrimary}25` }}>flipFuses()</code> then replace
                <code style={{ background: `${t.codeBg}cc`, color: t.inlineCodeText, padding: "2px 7px", borderRadius: 4, fontSize: 12, border: `1px solid ${t.accentPrimary}25` }}>app.asar</code> — that's it.
              </div>
                ...
                <thead>
                  <tr>
                    {["Layer", "Mechanism", "Bypass"].map(h => (
                      <th key={h} style={{ background: t.codeBg, color: t.accentPrimary, padding: "6px 10px", textAlign: "left", borderBottom: `1px solid ${t.borderColor}40`, fontWeight: 600 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0" }}>
              <div style={{ width: 16, height: 16, borderRadius: "50%", border: `2px solid ${t.accentPrimary}`, borderTopColor: "transparent", animation: "sparkle-spin 0.8s linear infinite" }} />
